...
exports.NewMigrations1740152566362 = void 0;
class NewMigrations1740152566362 {
    constructor() {
        this.name = 'NewMigrations1740152566362';
    }
    async up(queryRunner) {
        ...
        await queryRunner.query(`CREATE TABLE \`basket_product\` (\`id\` int NOT NULL AUTO_INCREMENT, \`count\` int NOT NULL DEFAULT '1', \`totalPrice\` int NOT NULL, \`variant\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`basketId\` int NULL, \`productId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`basket\` (\`id\` int NOT NULL AUTO_INCREMENT, \`quantity\` int NOT NULL, \`totalPrice\` int NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`product\` ADD CONSTRAINT \`FK_ff0c0301a95e517153df97f6812\` FOREIGN KEY (\`categoryId\`) REFERENCES \`category\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`basket_product\` ADD CONSTRAINT \`FK_d412cfb078de99eff78a68d850c\` FOREIGN KEY (\`basketId\`) REFERENCES \`basket\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`basket_product\` ADD CONSTRAINT \`FK_2cffcc93a389a40d4c0642d99ff\` FOREIGN KEY (\`productId\`) REFERENCES \`product\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    async down(queryRunner) {
        ...
        await queryRunner.query(`ALTER TABLE \`basket_product\` DROP FOREIGN KEY \`FK_2cffcc93a389a40d4c0642d99ff\``);
        await queryRunner.query(`ALTER TABLE \`basket_product\` DROP FOREIGN KEY \`FK_d412cfb078de99eff78a68d850c\``);
        await queryRunner.query(`ALTER TABLE \`product\` DROP FOREIGN KEY \`FK_ff0c0301a95e517153df97f6812\``);
        await queryRunner.query(`DROP TABLE \`basket\``);
        await queryRunner.query(`DROP TABLE \`basket_product\``);
...
exports.NewMigrations1740152566362 = NewMigrations1740152566362;
//# sourceMappingURL=1740152566362-NewMigrations.js.map